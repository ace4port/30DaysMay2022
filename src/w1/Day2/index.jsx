import React from 'react'
import { FlexColumn, Flex, PostAuthorStyled } from './styled'
import { colors } from '../../misc'

import Avatar, { BorderedAvatar, ChatAvatar } from './Avatar'

import img from '../../assets/Avatars/index.png'
import img2 from '../../assets/Avatars/bill.png'
import img3 from '../../assets/Avatars/shutter.png'

const Week2 = () => {
  return (
    <>
      <Flex>
        <FlexColumn>
          <Avatar img={img} name='placeholder' />
          <Avatar name='placeholder' />
        </FlexColumn>

        <FlexColumn>
          <BorderedAvatar name='placeholder' />
          <BorderedAvatar outline={colors.secondary} img={img2} name='placeholder' />
          <BorderedAvatar outline={colors.success} img={img3} name='placeholder' />
        </FlexColumn>

        <FlexColumn>
          <ChatAvatar img={img3} name='placeholder' />
          <ChatAvatar status='online' img={img3} name='placeholder' />
          <ChatAvatar status='away' img={img3} name='placeholder' />
        </FlexColumn>

        <FlexColumn>
          <Avatar img={img} name='placeholder' size='small' />
          <Avatar img={img} name='placeholder' size='large' />
          <Avatar img={img2} name='placeholder' size='large' />
          <BorderedAvatar
            img={
              'data:image/jpeg;base64,'
            }
            name='placeholder'
          />
          <BorderedAvatar
            img={
              'data:image/jpeg;base64,'
            }
            name='placeholder'
            size='large'
          />
        </FlexColumn>
      </Flex>

      <AdminAuthor img={img2} name='Billy' />

      <PostAuthor img={img2} name='Billy' time='12hr ago' />
    </>
  )
}

export default Week2

const AdminAuthor = ({ img, name }) => {
  return (
    <Flex alignItems='center'>
      <p>{name}</p>
      <Avatar img={img} name={name} />
    </Flex>
  )
}

export const PostAuthor = ({ img, name, time }) => {
  return (
    <PostAuthorStyled alignItems='center'>
      <Avatar img={img} name={name} />
      <p>
        {name}
        <span>{time}</span>
      </p>
    </PostAuthorStyled>
  )
}
